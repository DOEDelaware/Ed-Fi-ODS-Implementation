...
INSERT INTO edfi.Program (EducationOrganizationId, ProgramName, ProgramTypeDescriptorId)  --sk 3/30/2023
select LocalEducationAgencyId, 'Homeless', 
	(select DescriptorId from edfi.Descriptor where Namespace like '%ProgramTypeDescriptor%' and CodeValue='Homeless')
from edfi.LocalEducationAgency LEA
where not exists (select * from edfi.Program p where p.EducationOrganizationId=LEA.LocalEducationAgencyId and ProgramName='Homeless')
go
