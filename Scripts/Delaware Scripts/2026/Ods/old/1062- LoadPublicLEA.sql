...
--begin
 ...
 and districtcode not in (select EducationOrganizationId from edfi.EducationOrganization)
 and ACT='A'
go
...
select
 ...
 and isnull(Street1,'')<>'' and isnull(City,'')<>'' and isnull(Zip,'')<>''  --business rule in API causes writing this row to fail with blank address info
...
select 
 ...
 and districtcode in (select EducationORganizationId from edfi.EducationOrganization) 
  and districtcode not in (select localeducationagencyid from edfi.LocalEducationAgency)
 and districtcode<>95--state org
 and dbo.descriptor('%LocalEducationAgencyCategory%',Type) is not null
  and ACT='A'
